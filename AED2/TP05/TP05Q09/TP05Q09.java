/**
 *---------------------------------------------------------------------------
 * TP 05 - QUESTÃO 09 - Ordenação por Counting Sort
...
class ListaPessoa {
	...
	public void CoutingSort() {
		...
		Pessoa[] ordenado = new Pessoa[cout];

		for(int i = 0; i < count.length; count[i] = 0, i++);

		for(int i = 0; i < cout; count[getAno(i)]++, i++);

		for(int i = 1; i < count.length; count[i] += count[i-1], i++);

		for(int i = cout-1; i >= 0; ordenado[count[getAno(i)]-1] = arrayList[i], count[getAno(i)]--, i--);

     	for(int i = 0; i < cout; arrayList[i] = ordenado[i], i++);
