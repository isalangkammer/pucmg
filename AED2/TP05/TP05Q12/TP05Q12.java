/**
 *---------------------------------------------------------------------------
 * TP 05 - QUESTÃO 12 - Ordenação por radixsort
...
class ListaPessoa {
    ...
    public void Bolha() {
        ...
        for(int i = cout-1; i > 0; i--) {
            ...
            for(int j = 0; j < i; j++) {
                ...
                if(doidao(getAno(j), j) > (doidao(getAno(j+1), j+1))) {
    ...
    public int doidao(int index, int i) {

        int d = (1000 * index ) + arrayList[i].getId();
        return d;
    ...
    private int getAno(int index) {
        ...
        String data = arrayList[index].getDataMorte();
