/**
 *---------------------------------------------------------------------------
 * TP 05 - QUESTÃO 07 - Ordenação por heapsort
...
class ListaPessoa {
	...
	public void constroi(int tamanho) {
		...
		for(int i = tamanho; i > 1 && getDia(i) > getDia(i/2); i = i/2) {
	...
	public void reconstroi(int tamanho) {
		...
		while(i <= (tamanho/2)) {
			...
			if(getDia(2*1) > getDia(2*i+1) || 2*i == tamanho) {
	...
	public void heapsort() {
		...
		Pessoa[] array = new Pessoa[arrayList.length + 1];

		for(int i = 0; i < arrayList.length; i++) {

			array[i+1] = arrayList[i];
			
		}//end for
		
		arrayList = array;

		for(int tamanho = 2; tamanho <= cout; tamanho++) {
		...
		array = arrayList;
		arrayList = new Pessoa[arrayList.length];
