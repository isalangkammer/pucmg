...
class ListaPessoa {
	...
	public void reconstroi(int tamanho) {
		...
		int tmp1 = getDia(2*1);
		int tmp2 = getDia(2*i+1);

			if(tmp1 > tmp2 || 2*i == tamanho) {
	...
	public void heapsort() {
		...
		for(int tamanho = 2; tamanho <= arrayList.length; tamanho++) {

			constroi(tamanho);
			
		}//end for

		int tamanho = arrayList.length;
	...
	private int getDia(int index){
		...
		return Integer.parseInt(arrayList[index].getDataNasc().substring(0,2).trim());
