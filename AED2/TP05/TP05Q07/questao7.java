/**
 *---------------------------------------------------------------------------
 * TP 05 - QUESTÃO 08 - Ordenação por quicksort
...
class ListaPessoa {
	...
	/**
	 * Método de ordenação por insercao
	 */
	public void Quicksort(int esq, int dir) {
		...
		int i = esq, j = dir;
		int pivo = getDia((dir+esq)/2);

		while(i <= j) {

			//System.out.println(getAno(i) + " 0");
			//System.out.println(getAno(j) + " 01");
			//System.out.println(i + " i atual");
			//System.out.println(j + " j atual");
			...
			if(i <= j) {
				...
				//System.out.println(getAno(i) +" "+ getAno(j));
	...
	public void desempate() {
		...
		for (int i = 0;  i < cout-1; i++ ) {
			...
			if(getDia(i) == getDia(i+1)){
			
				if(arrayList[i].getId() > arrayList[i+1].getId()){
	...
	public void swap(int i, int j) {
		...
		Pessoa tmp = arrayList [j];
		arrayList[j] = arrayList[i];
		arrayList[i] = tmp;

	}//end swap 

	private int getDia(int index){

		String data = arrayList[index].getDataNasc();
		return data.contains("ca") ? 0 : Integer.parseInt(data.substring(0,2).trim());
...
public class TP05Q07 {
	...
	public static void main(String[] args) {
		...
		Pessoa pessoa = new Pessoa();
		ListaPessoa listapessoa = new ListaPessoa(40);
		String nomeArquivo = "";
		...
		//System.out.println(listapessoa.cout);
		listapessoa.Quicksort();
		listapessoa.desempate();
		listapessoa.mostrar();
