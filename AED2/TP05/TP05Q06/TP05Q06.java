/**
 *---------------------------------------------------------------------------
 * TP 05 - QUESTÃO 06 - Ordenação por shellsort
...
class ListaPessoa {
	...
	public void InsercaoPorCor(int cor, int h) {
		...
		for(int i = (h + cor); i < cout; i = i + h) {
			
			Pessoa temp = arrayList[i];
			String tmp = arrayList[i].getDataNasc();
			int j = i - h;

			while(j >= 0 && arrayList[j].getDataNasc().compareTo(tmp) > 0) {

				arrayList[j + h] = arrayList[j];
				j = j - h;
