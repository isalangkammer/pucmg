/**
 *------------------------------------------------------------------------
 * TP 03 - QUESTÃO 04 - Fila Circular com Alocação Sequencial
...
class FilaPessoa {
	...
	public void Inserir(Pessoa pessoa) {
		...
		if(((ultimo + 1) % arrayFila.length) == primeiro) {

			Remover();

		}//end if

		arrayFila[ultimo] = pessoa;
		ultimo = (ultimo + 1) % arrayFila.length;
	...
	public Pessoa Remover() {
		...
		Pessoa tmp = arrayFila[primeiro];
		primeiro = (primeiro + 1) % arrayFila.length;
	...
	public void mostrar() {
		...
		while(i != ultimo) {
			...
			MyIO.println(arrayFila[i].getNome() + " $ " + arrayFila[i].getIdade());
			i = ((i+1) % arrayFila.length);
	...
	public double Media() {
		...
		for(int i = primeiro; i != ultimo; i = ((i + 1) % arrayFila.length)) {

			soma = soma + arrayFila[i].getIdade();
			cont++;
...
public class TP03Q04 {
	...
	public static void main(String[] args) throws Exception {
		...
		Pessoa pessoa = new Pessoa();
		FilaPessoa filapessoa = new FilaPessoa(5);
		String nomeArquivo = "";
		int aux = 0, media = 0;
