/**
 *---------------------------------------------------------------------------
 * TP 06 - QUESTÃO 04 - Quicksort com lista duplamente encadeada
		...
		for(int i = 0; i < aux; i++) {
			...
			switch(nomeArquivo.substring(0, 2)) {
				...
				case "II" : 
					...
					listadupla.InserirInicio(pessoa);
					break;
				...
				case "I*" :
					...
					listadupla.Inserir(pessoa, posicao);
					break;

				case "RI" :

					MyIO.println("(R) " + listadupla.RemoverInicio().getNome());
					break;

				case "RF" :

					MyIO.println("(R) " + listadupla.RemoverFim().getNome());
					break;
				
				case "R*" :

					posicao = Integer.parseInt(nomeArquivo.substring(2).trim());
					MyIO.println("(R) " + listadupla.Remover(posicao).getNome());
					break;
